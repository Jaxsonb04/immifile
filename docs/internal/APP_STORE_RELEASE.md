...
The release stores account contact information, an internal user/session identifier, security metadata, user-entered receipt numbers/status notes, the assistant daily counter, and the explicit OpenAI consent choice for app functionality. Current and recent user messages are sent to OpenAI only after consent and are not stored as a server-side transcript by Immifile. OpenAI may retain prompts and replies for up to 30 days in abuse-monitoring logs, with longer legal or safety retention possible. The app has no ads or cross-app tracking. After account deletion, an opaque deletion-protection identifier normally remains for approximately one hour solely to reject stale-session requests and safely finish deletion; it contains no saved case content. Interrupted provider/session cleanup may retain it longer only until that security cleanup safely completes, after which it is removed. Confirm the final answers against the production Convex, Better Auth, and OpenAI configuration before submission.
